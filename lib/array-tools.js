...
You can also chain together operations. Any method from array-tools or `Array.prototype` which returns an array can be chained. Methods which do not return an arrary (e.g. {@link module:array-tools.exists} or `Array.prototype.join`) can not be chained. If the final operation in your chain is one of the chainable methods then you must terminate it with `.val()` to obtain the value. For example: 
...
["exists", "findWhere", "last"].forEach(function(method){
    ArrayTools.prototype[method] = function(){
        var args = arrayify(arguments);
		args.unshift(this._data);
