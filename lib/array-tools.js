...
var reduceUnique = require('reduce-unique')
var filterWhere = require('filter-where')
var reduceExtract = require('reduce-extract')
...
function where (array, query) {
  return arrayify(array).filter(filterWhere(query))
}
...
function unique (array) {
  return arrayify(array).reduce(reduceUnique, [])
}
...
function extract (array, query) {
  return array.reduce(reduceExtract(query), [])
}
...
function flatten (array) {
  return arrayify(array).reduce(reduceFlatten, [])
}
